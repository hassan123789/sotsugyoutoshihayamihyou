        ...
        {result && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(44, 82, 130, 0.08)' }}>
              <div className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>{t.currentAgeLabel}</div>
              <div className="text-2xl font-bold" style={{ color: 'var(--color-primary)' }}>
                {result.age}{t.ageUnit}
              </div>
            </div>
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(213, 63, 140, 0.08)' }}>
              <div className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>{t.eto}</div>
            ...
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(44, 82, 130, 0.08)' }}>
              <div className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>{t.zodiac}</div>
              <div className="text-2xl font-bold" style={{ color: 'var(--color-primary)' }}>
                {result.zodiac.emoji}
              </div>
              <div className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
            ...
            <div className="p-4 rounded-xl text-center" style={{ background: 'rgba(213, 63, 140, 0.08)' }}>
              <div className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>{t.warekiTitle}</div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: 'var(--color-border-light)' }}>
                  <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--color-text)' }}>{t.yearHeader}</th>
                  <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--color-text)' }}>{t.warekiTitle}</th>
                  <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--color-text)' }}>{t.currentAgeLabel}</th>
                  <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--color-text)' }}>{t.eto}</th>
                </tr>
              </thead>
              <tbody>
                {ageTable.slice(0, 60).map((row) => (
                  <tr key={row.year} className="border-t" style={{ borderColor: 'var(--color-border)' }}>
                    <td className="px-4 py-2 font-medium" style={{ color: 'var(--color-text)' }}>{row.year}{t.yearUnit}</td>
                    <td className="px-4 py-2" style={{ color: 'var(--color-text-secondary)' }}>{row.wareki}</td>
                    <td className="px-4 py-2 font-medium" style={{ color: 'var(--color-primary)' }}>{row.age}{t.ageUnit}</td>
