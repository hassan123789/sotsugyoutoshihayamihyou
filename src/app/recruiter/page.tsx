...
export default function RecruiterPage() {
  ...
  const [schoolType, setSchoolType] = useState<'university4' | 'university6' | 'master' | 'highschool' | 'junior'>('university4');
  ...
  }, [graduationYear, graduationMonth, schoolType]);
    ...
    <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-14">
      ...
      <header className="text-center mb-10">
        <div 
          ...
          style={{
            background: 'linear-gradient(135deg, var(--color-primary) 0%, #38B2AC 100%)',
        ...
        <div className="card p-6">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setGraduationYear(parseInt(e.target.value) || currentYear)}
                  className="input-field flex-1"
                  min={1950}
                  max={2100}
                />
                <span className="flex items-center px-3" style={{ color: 'var(--color-text-secondary)' }}>年</span>
                <select
                  value={graduationMonth}
                  onChange={(e) => setGraduationMonth(parseInt(e.target.value))}
          ...
          {estimatedBirth && (
            <div 
              ...
              style={{ 
                background: 'linear-gradient(135deg, rgba(44, 82, 130, 0.1) 0%, rgba(56, 178, 172, 0.1) 100%)',
              ...
              <div className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium" style={{ color: 'var(--color-text)' }}>
                    {estimatedBirth.latest.year}{t.years}{estimatedBirth.latest.month}{t.months}{estimatedBirth.latest.day}{t.days}まで
                    <span className="ml-2 text-sm" style={{ color: 'var(--color-text-muted)' }}>
                      ({toWareki(estimatedBirth.latest.year, estimatedBirth.latest.month)}年)
                    </span>
                  </span>
                </div>
                <div className="flex justify-between">
                  ...
                  <span className="font-medium" style={{ color: 'var(--color-text)' }}>
                    {estimatedBirth.earliest.year}{t.years}{estimatedBirth.earliest.month}{t.months}{estimatedBirth.earliest.day}{t.days}以降
                    <span className="ml-2 text-sm" style={{ color: 'var(--color-text-muted)' }}>
                      ({toWareki(estimatedBirth.earliest.year, estimatedBirth.earliest.month)}年)
                    </span>
                  </span>
                </div>
                ...
                <div className="flex justify-between">
                  ...
                  >
                    {calculateAge(estimatedBirth.earliest.year, 4, 2)}〜{calculateAge(estimatedBirth.latest.year, 4, 1)}{t.ageUnit}
                  </span>
                </div>
              </div>
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '2px solid var(--color-border)' }}>
                <th className="py-3 px-4 text-left" style={{ color: 'var(--color-text-secondary)' }}>
                  {t.fiscalYear}
                </th>
                <th className="py-3 px-4 text-left" style={{ color: 'var(--color-text-secondary)' }}>
                  {t.birthYearRange}
                </th>
                <th className="py-3 px-4 text-right" style={{ color: 'var(--color-text-secondary)' }}>
                  {t.currentAge}
                </th>
              </tr>
            </thead>
              ...
              {graduationTable.map((row, index) => (
                <tr 
                  ...
                  style={{ 
                    ...
                    background: row.graduationYear === currentYear 
                      ? 'rgba(44, 82, 130, 0.1)' 
                      : index % 2 === 0 
                        ? 'transparent' 
                        : 'var(--color-card)'
                ...
                >
                  ...
                  <td className="py-3 px-4 text-right font-medium" style={{ color: 'var(--color-primary)' }}>
                    {row.currentAge}〜{row.currentAge + 1}{t.ageUnit}
                  </td>
                </tr>
              ))}
            </tbody>
