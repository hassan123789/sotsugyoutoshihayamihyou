...
function getSchoolIcon(schoolName) {
    if (schoolName.includes('小学校'))
        return '🎒';
    if (schoolName.includes('中学校'))
        return '📚';
    if (schoolName.includes('高等学校'))
        return '🏫';
    if (schoolName.includes('大学院'))
        return '🎓';
    if (schoolName.includes('大学') || schoolName.includes('専門'))
        return '🎓';
    return '📖';
}
/** タイムライン形式で学歴を表示 */
